...
| `shops/<name>.html` | One page per counter (bakery, butchery, deli, fish, fruit, cellar, eatery, living, cleaning, gifts) with a sticky counters bar linking across pages |
...
Below 820px the top bar is removed (language, dark mode and the live open/closed status move into the menu drawer) and the site becomes app-like rather than a squeezed desktop page: a bottom tab bar (Home, Counters, Offers, Search, Basket, Account) with an animated active indicator and live basket badge; counter chips that slide in under the hero instead of the sticker cluster; centred snap carousels for counters, departments, tiles, articles and feature cards — the active card sits exactly in the middle with its neighbours peeking in, with progress dots; "How it works" becomes a swipeable journey of four photo cards with a progress bar and animated chips (basket items, collection slot, +points and the reward); a basket that rises as a bottom sheet; a full-screen search with popular terms and counter shortcuts; a floating "n items · €total · View basket" pill; 2-up product cards with a round "+" add button and fly-to-basket towards the tab bar; tap-press feedback; a back-to-top button; compact top bar and header that hides while scrolling down. "How it works" becomes a vertical stepper and long copy is trimmed.
...
## Staff panel — `admin/index.html`
A standalone admin for personnel (PIN `1963`, "Staff" link in the footer). Views: **Dashboard** (today's revenue, open orders, deliveries, low stock, 14-day revenue bars, sales by counter, attention lists), **Orders** (search/filter, inline status changes, order detail with progress, driver assignment, print slip, CSV export), **Deliveries** (driver availability, kanban: to prepare → packing → on the road → delivered), **Products** (search, add / remove; tap any product to open its full editor: EN/EL names, counter, unit, badge, EN/EL description override, photo by URL or upload; pricing with cost price and live margin, member price, discount % ↔ was-price, quick −10/−15/−20/−25/−30% buttons, offer end date that expires automatically; inventory with stock, reorder level, target stock, supplier, lead time, "receive stock" restock with unit cost, movement log; sales last 7/30 days and estimated days of stock; hide/publish; changes go live on the storefront at once), **Inventory** (stock bars, ± adjustments, reorder levels, low-stock filter, reorder-list CSV, full stock count with movement log). Starting an order deducts stock automatically. Orders placed on the storefront appear as "new"; "Sync storefront orders" pulls any missed ones. Data lives in localStorage (demo); "Reset demo data" restores it.
